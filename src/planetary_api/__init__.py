...
def users():
    ...
    results = users_schema.dump(users_list)
    return jsonify(results)

@app.route('/planets', methods=['GET'])
def planets():
    planets_list = Planet.query.all()
    results = planets_schema.dump(planets_list)
    return jsonify(results)
...
def retrieve_email(email: str):
    ...
    if user:
        msg = Message(
            subject='Email password retrieval: ' + user.password,
            sender='admin@planetary-api.com',
            recipients=[email]
            )
        mail.send(message=msg)
        return jsonify(message='Email sent to ' + email)
    else:
...
def update_planet():
    ...
    if planet:
        ...
        return jsonify(message='Planet was updated.'), 202
    else:
        return jsonify(message='Planet does not exist.'), 404
